...
- ✅ Piece-square tables — Simplified Evaluation Function (Michniewski) values for all six piece types. White-perspective; black looks up `sq ^ 56` (rank flip). Middlegame king table only — tapered eval (interpolating MG↔EG by remaining material) still on the list. Startpos opening move is now Nb1-c3 (real chess) rather than Nb1-a3 (bad — "knight on the rim").
- Tapered eval — interpolate middlegame vs endgame PSTs by remaining non-pawn material so the king centralizes correctly in endgames
