...
//
// Tables are mutable so the SPSA tuner (see tune.cpp) can adjust them.
// MG and EG tables are separate for every piece — Michniewski used
// identical MG/EG for P/N/B/R/Q; splitting them lets the tuner discover
// phase-specific placement value (e.g., "a knight at D4 is worth more
// mid-game than endgame"). King already had split tables. Callers must
// invoke Position::recompute_psq() on any position they consume the
// eval of after mutating a PST value.
//
// The MG-side arrays are populated with Michniewski's original values;
// the EG-side arrays start as copies of MG (for P/N/B/R/Q) and use
// Michniewski's activity table for KING.

int pst_mg[NUM_PIECE_TYPES][NUM_SQUARES] = {
    // NO_PIECE_TYPE — sentinel; never looked up because pt is always in
    // [PAWN..KING] for real pieces on the board.
    { 0 },
    // PAWN
    {
         0,  0,  0,  0,  0,  0,  0,  0,
    ...
    // KING (middlegame — safety, castled squares rewarded)
    {
...
int pst_eg[NUM_PIECE_TYPES][NUM_SQUARES] = {
    { 0 },
    // PAWN (starts as copy of MG — tuner discovers EG-specific structure)
    {
         0,  0,  0,  0,  0,  0,  0,  0,
         5, 10, 10,-20,-20, 10, 10,  5,
         5, -5,-10,  0,  0,-10, -5,  5,
    ...
    // KING (endgame — activity, centralization rewarded)
    {
